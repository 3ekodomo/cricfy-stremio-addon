...
async function getStreamUrl(id) {
  ...
  if (channel && channel.url) {
    ...
    if (channel.license_string) {
      const hexPairRegex = /^[0-9a-fA-F]+:[0-9a-fA-F]+$/;

      stream.behaviorHints.notWebReady = true;

      if (hexPairRegex.test(channel.license_string)) {
        stream.behaviorHints.drm = {
          type: "clearKey",
          clearKeys: {
            [channel.license_string.split(':')[0]]: channel.license_string.split(':')[1]
          }
        };
      } else if (channel.license_string.startsWith('http')) {
        let licenseUrl = channel.license_string.split('|')[0];
        stream.behaviorHints.drm = {
          type: "clearKey",
