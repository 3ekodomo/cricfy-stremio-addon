...
// Secrets injected via environment variables or read from files
let SECRET1 = process.env.CRICFY_SECRET1 || '';
let SECRET2 = process.env.CRICFY_SECRET2 || '';

try {
  if (!SECRET1) {
    SECRET1 = fs.readFileSync(path.join(__dirname, '../resources/secret1.txt'), 'utf8').trim();
  }
} catch (e) {
  // ignore
}
...
try {
  if (!SECRET2) {
    SECRET2 = fs.readFileSync(path.join(__dirname, '../resources/secret2.txt'), 'utf8').trim();
  }
} catch (e) {
  // ignore
}
